...
/**
 * Search all the MX servers related to the SLDs.
 */
public class MxSearcher {
    private static final Logger logger = Logger.getLogger(MxSearcher.class.getCanonicalName());
    ...
    public void execute() throws SQLException, InterruptedException {
        ...
        for (InetSocketAddress server : servers) {
            for (InetAddress local : server.getAddress() instanceof Inet4Address ? locals4 : locals6) {
                for (int k = 0; k < count; k++) {
                    Thread thread = new ResolverThread(server, Type.MX, inQueue, outQueue, sentinel, local);
                    thread.start();
                    threads.add(thread);
                }
            }
        ...
        // --- Create mx records ---
        sql = sqlProvider.getSql(getClass(), "create_mx");
        try (Connection con = database.getConnection();
             Statement stmt = con.createStatement()) {
            stmt.execute(sql);
            con.commit();
    ...
    private Runnable createDatabaseRunnable(SynchronousQueue<Message> outQueue) {
        return () -> {
            ...
            String sql1 = sqlProvider.getSql(getClass(), "insert_record_mx");
            String sql2 = sqlProvider.getSql(getClass(), "update_record_searched");
            try (Connection con1 = database.getConnection();
                ...
                while (true) {
                    ...
                    for (Record r : message.getSection(Section.ANSWER)) {
                        if (r.getName().equals(name) && r.getType() == Type.MX) {
                            list.add(r);
                        }
                    }
                    if (list.size() == 0) {
                    ...
                    for (Record record : list) {
                        ...
                        String mx = ((MXRecord) record).getTarget().toString().toLowerCase();
                        stmt1.setString(1, owner);
                        stmt1.setString(2, mx);
                        stmt1.addBatch();
